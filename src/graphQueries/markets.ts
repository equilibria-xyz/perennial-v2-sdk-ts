...
export const QueryMarketAccumulationData = gql(`
  query QueryMarketAccumulationData($markets: [Bytes!]!, $from: BigInt!, $to: BigInt!) {
    marketAccumulations(
      where:{ bucket: hourly, market_: { id_in: $markets }, timestamp_gte: $from, timestamp_lte: $to }
      orderBy: timestamp
      orderDirection: asc
    ) {
      market { id }, timestamp, longNotional, shortNotional, fundingRateMaker, fundingRateLong, fundingRateShort, interestRateMaker, interestRateLong, interestRateShort
    }

    startCurrentAccumulations: markets(where: { id_in: $markets }) {
      id
      startAccumulation: accumulators(where: { toVersion_gte: $from }, first: 1, orderBy: toVersion, orderDirection: asc) { fundingMaker, interestMaker, positionFeeMaker, exposureMaker, toVersion }

      currentAccumulation: accumulators(where: { toVersion_lte: $to }, first: 1, orderBy: toVersion, orderDirection: desc) { fundingMaker, interestMaker, positionFeeMaker, exposureMaker, toVersion }
    }
  }
`)
