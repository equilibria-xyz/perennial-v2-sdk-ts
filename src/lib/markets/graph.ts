...
import { OrderDataFragment, PositionDataFragment } from '../../types/gql/graphql'
import { Day, last7dBounds, last24hrBounds, nowSeconds } from '../../utils'
...
export async function fetchActivePositionsPnl({
  ...
  const positionPnls = marketsWithAddresses.map(({ asset: market, marketAddress }) => {
    ...
    if (graphMarketAccount && graphPosition) {
      ...
      processedGraphPosition.collateralAccumulations = Object.values(accumulationToGlobal).reduce(
        (acc, { unrealized, type }) => {
          acc[type] += unrealized
          return acc
        },
        processedGraphPosition.collateralAccumulations,
      )
    ...
    return {
      ...
      startCollateral,
      totalAccumulatedCollateral: pendingTradeImpactAsOffset,
      totalAccumulatedFees: pendingTradeFee + pendingOrderSettlementFee,
      collateralAccumulations: {
        offset: pendingTradeImpactAsOffset,
        pnl: 0n,
        funding: 0n,
        interest: 0n,
...
function processGraphPosition(
  ...
  let totalAccumulatedCollateral = BigOrZero(graphPosition.accumulation.collateral_accumulation)
  let totalAccumulatedFees = BigOrZero(graphPosition.accumulation.fee_accumulation)
  const collateralAccumulations = accumulateRealized([graphPosition.accumulation])
  ...
  if (side === 'maker') {
    const offsetAsFee = collateralAccumulations.offset * -1n // Convert offset to a fee
    feeAccumulations.trade += offsetAsFee // Convert offset to a fee
    totalAccumulatedFees += offsetAsFee

    collateralAccumulations.offset = 0n
    totalAccumulatedCollateral -= collateralAccumulations.offset
  }
  ...
  if (pendingPositionData) {
    ...
    returnValue.totalAccumulatedFees += totalFee
    if (side === 'maker') {
      returnValue.feeAccumulations.trade += pendingPositionData.offset
      totalFee += pendingPositionData.offset
    } else {
      returnValue.collateralAccumulations.offset += pendingPositionData.offset
      returnValue.totalAccumulatedCollateral += pendingPositionData.offset
...
function processOrder(order: OrderDataFragment) {
  ...
  const returnValue = {
    ...
    netPnlPercent: percentDenominator !== 0n ? Big6Math.div(netPnl, percentDenominator) : 0n,
    // Accumulation Breakdowns
    grossPnl: BigInt(order.accumulation.collateral_accumulation),
    totalFees: BigInt(order.accumulation.fee_accumulation),
    collateralAccumulations: accumulateRealized([order.accumulation]),
...
export async function fetchMarkets24hrData({
  ...
  const { marketAccumulations } = await graphClient.request(QueryMarketAccumulationData, {
    markets: marketAddresses.map((m) => m.marketAddress),
    from: from.toString(),
    to: to.toString(),
  })

  return marketAddresses.reduce(
    (acc, v) => {
      const marketVolumes = marketAccumulations.filter((vol) => getAddress(vol.market.id) === v.marketAddress)
      acc[v.asset] = marketVolumes.map((vol) => ({
        periodStartTimestamp: vol.timestamp,
        longNotional: vol.longNotional,
        shortNotional: vol.shortNotional,
        market: v.marketAddress,
      }))
    ...
    {} as Record<
      SupportedAsset,
      { periodStartTimestamp: string; longNotional: string; shortNotional: string; market: Address }[]
    >,
  )
}
/**
 ...
 */
export async function fetchMarkets7dData({
  chainId,
  graphClient,
  markets,
}: {
  ...
  const marketAddresses = chainAssetsWithAddress(chainId, markets)
  const { marketAccumulations, startCurrentAccumulations } = await graphClient.request(QueryMarketAccumulationData, {
    markets: marketAddresses.map((m) => m.marketAddress),
    from: from.toString(),
    to: to.toString(),
  })
  ...
  const parsedData = marketAddresses.map(({ asset: market, marketAddress }) => {
    const startAccumulator = startCurrentAccumulations
      .find((m) => getAddress(m.id) === marketAddress)
      ?.startAccumulation.at(0)
    const currentAccumulator = startCurrentAccumulations
      .find((m) => getAddress(m.id) === marketAddress)
      ?.currentAccumulation.at(0)

    // Scale accumulation values to fill the 7d window
    const accumulatorScaleFactor = Big6Math.fromFloatString(
      (Number(7n * Day) / Number(to - Number(startAccumulator?.toVersion ?? from))).toString(),
    )
    const takerVolumes = marketAccumulations.map((vol) => ({
      long: BigInt(vol.longNotional),
      short: BigInt(vol.shortNotional),
    }))
    const fundingRates = marketAccumulations.map((rate) => ({
    ...
    return {
      ...
      makerAccumulation: {
        funding: Big6Math.mul(
          BigOrZero(currentAccumulator?.fundingMaker) - BigOrZero(startAccumulator?.fundingMaker),
          accumulatorScaleFactor,
        ),
        interest: Big6Math.mul(
          BigOrZero(currentAccumulator?.interestMaker) - BigOrZero(startAccumulator?.interestMaker),
          accumulatorScaleFactor,
        ),
        positionFee: Big6Math.mul(
          BigOrZero(currentAccumulator?.positionFeeMaker) - BigOrZero(startAccumulator?.positionFeeMaker),
          accumulatorScaleFactor,
        ),
        exposure: Big6Math.mul(
          BigOrZero(currentAccumulator?.exposureMaker) - BigOrZero(startAccumulator?.exposureMaker),
          accumulatorScaleFactor,
        ),
      },
    }
